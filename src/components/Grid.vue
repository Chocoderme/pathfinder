...
const handlePointerEnter = (x: number, y: number) => {
  ...
  if (
    ...
    Math.abs(lastPointerEnterPos.y - y) + Math.abs(lastPointerEnterPos.x - x) >
      1
  ) {
          ...
          if (Math.round(j) === Math.round(slope * i + b)) {
            if (
              gridStore.cell(Math.round(i), Math.round(j))?.[0] ===
                CellType.START ||
              gridStore.cell(Math.round(i), Math.round(j))?.[0] === CellType.END
            )
              continue;
